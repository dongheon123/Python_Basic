...
# ** 선택자
#   1. 자식 선택자: div > span (꺽쇠가 있으면 내 아래 1개만 찾는거)
#   2. 자손 선택자: div span(꺽쇠가 없으면 내 아래 모든거 다 찾는거)
#   3. 아이디 선택자: #t1 (유니크 1개)
...
# 1. 선택자(id,class)
# 2. 상위 관계(자식,자손)
content_list = doc.select("div.article_view p") #태그이름으로 select 하면 안됨 ex)doc.select("div")
