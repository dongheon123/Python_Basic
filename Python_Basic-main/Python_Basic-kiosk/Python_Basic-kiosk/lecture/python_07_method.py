...
#   + 함수 내용은 가능하면 짧게 작성(Code Line 줄이기) -> 기능별로 따로 따로 만들기
#   ex) def print_hello():
#           print("Hello")

# 2. 함수의 역할
#   + 하나의 함수에 유사한 역할을 하는 코드만 포함
#   + 하나의 함수는 한가지 기능만 명확히 정의 (덧셈 ,뺼셈 ,나눗셈 같이 기능이 다르면 여러개 만들어라!)
...
# 3. 인자(argment or parameter) 정의: 함수 입력값
# 4. return: 함수 종료 의미
# 5. return 반환값: 함수종료+호출문으로 반환값 전달(typle)
...
# 함수가 끝나면 함수가 호출되었던 곳으로 돌아간다.
# sum_two_value 함수는 return 값이 없으므로 None값이 반환된다.
print(result)
...
# - 심지어 사용자 정의함수를 parameter로 전달 가능
...
# defalut Parameter는 뒤에서 부터 시작해야한다. 아니면 오류 발생

# 5. return
# - 기본적으로 함수 종료 의미
# - return 반환값: 함수호출문으로 값 전달(tuple type)
# - return만 사용하면 함수호출문으로 None값 전달
# - return이 없는 경우 들여쓰기 종료 함수 종룔로 간주
...
#   - 같은 이름의 지역변수와 전역변수 존재하는 경우 가까운(지역변수) 우선순위가 높음
num1 = 10 #전역
num2 = 20 #전역
...
# 7. Variable length parameter(가변 길인인자)
